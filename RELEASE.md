...
### N-2.1. 防止重複領取免費試用(濫用防範)

> 「怎麼防範用戶一直刷新 7 天免費試用?」——結論:**Google Play 在帳號層級自動擋,
> 我們 App 端沒有可被重置的試用邏輯**,幾乎不用寫程式。此節說明機制與殘留風險。

1. **Play 帳號級資格(主要防線)**:免費試用是 Play 訂閱的「**優惠(offer)**」,資格已設成
   「**新訂閱者 / 從未訂閱過**」(見 N-2 步驟 3)。Play 在 **Google 帳號層級**記錄試用使用紀錄,
   **跨重裝、換機、取消後再訂都記得**;用過試用的帳號**不會再被授予試用**,會直接照原價收費。
   → 同一帳號沒有「刷新試用」的空間。
2. **App 端無 client-side 漏洞**(已稽核 `billing/EntitlementManager.kt`、`PaywallViewModel.kt`):
   - 試用**完全由 Play 發放**;App 內 `PlanOption.hasFreeTrial` **只用來顯示「7 天試用」標籤**,
     不發放任何權益。
   - `entitlement` **只認 Play 真實購買**(DataStore 快取僅在 Play 回報 Premium 時寫入 + 即時
     `refresh()` 查 Play)。**清除 App 資料 / 重新安裝不會重置試用**——試用記在使用者的 Google
     帳號,不在 App。→ 客戶端沒有可被重置或偽造的點。
3. **殘留風險 + 緩解**:唯一擋不住的是「**同一人開多個 Google 帳號**(各自綁新的付款方式)
   重複領」。緩解:
   - Play 自家的防濫用 / 詐騙偵測;試用**要求預先綁定付款方式**(提高門檻)。
   - 試用只掛在**年繳**方案 → 試用結束未取消會被收**一整年**,濫用者每週都得「換帳號 + 換卡 +
     試用前取消」,極不划算。
   - 若要更嚴可:縮短或移除試用、僅保留年繳試用、或加入伺服器端驗證(本 App 為**離線優先、
     無伺服器**,故不採用)。
4. **附帶澄清(與試用不同)**:血糖「**每位成員免費 10 筆**」(`FREE_GLUCOSE_LIMIT`)是
   **本地計數**,重裝會重置——但重裝同時**刪光所有資料**(自損),且 App 離線優先,此類免費額度
   本質上就是本地的。列為**已知且可接受**,非試用問題。付費閘總覽見 **N-5.1**。
