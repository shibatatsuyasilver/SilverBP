 ...
 * owner row is created by MIGRATION_17_18 and never deleted, so the cache can't
...
class MemberRepository(private val dao: MemberDao) {
    ...
    /**
     * The owner member's id. Memoized after the first read. Falls back to
     * synthesizing an owner only in the pathological case where the migration
     * backfill never ran (e.g. a brand-new in-memory DB in tests) so callers
     * always get a stable, valid id.
     */
    suspend fun ownerId(): String {
        cachedOwnerId?.let { return it }
        val existing = dao.getOwner()
        val id = existing?.id ?: ensureOwner().id
        cachedOwnerId = id
        return id
    }
