...
/**
 * One-way mirror sink for weight readings, implemented by the integration
 * track's `health.HealthConnectWeightBridge`. Declared here (not imported from
 * `health`) so the v19 data layer compiles standalone before the bridge lands;
 * [WeightRepository] accepts a nullable instance, exactly like [BpRepository]
...
/**
 * Member-scoped read/write access to weight readings, with the owner-only
