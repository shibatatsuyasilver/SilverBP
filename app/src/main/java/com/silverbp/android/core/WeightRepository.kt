...
/**
 * One-way mirror sink for weight readings, implemented by the integration track's
 * `health.HealthConnectWeightBridge`. Declared here (not imported from `health`)
 * so the v20 data layer compiles standalone before the bridge lands;
 * [WeightRepository] accepts a nullable instance, exactly like [GlucoseRepository]
 * takes a nullable `GlucoseHealthConnectBridge`.
 */
interface WeightHealthConnectBridge {
    /** Write one reading; returns the HC record id on success, null on any failure. */
    suspend fun write(reading: WeightReading): String?
}

/**
 * Member-scoped read/write access to weight readings, with the owner-only Health
 * Connect mirror guard. Mirrors [GlucoseRepository] exactly: the mirror only
 * fires for the owner member's readings (writing a family member's weight into
 * the device owner's Google Health would be a correctness / privacy bug, roadmap
 * §3-5 / §4-4), and [WeightDao.findUnmirrored] is already owner-filtered so the
...
class WeightRepository(
    ...
    // ============================================================
    // Member-scoped reads (v20) — the primary API.
    // ============================================================

    fun observeLatest(memberId: String): Flow<WeightReading?> =
        dao.observeLatest(memberId).map { it?.toDomain() }

    fun observeAll(memberId: String): Flow<List<WeightReading>> =
        dao.observeAll(memberId).map { list -> list.map { it.toDomain() } }

    fun observeRange(memberId: String, from: Instant, to: Instant): Flow<List<WeightReading>> =
    ...
    suspend fun upsert(reading: WeightReading) {
        ...
        // stamp the returned record id back so the weight sync worker won't
        // retry it. Owner-only guard (roadmap §3-5): a family member's weight
    ...
    /** Owner-only retry set for the Health Connect mirror (mirrors [GlucoseRepository]). */
