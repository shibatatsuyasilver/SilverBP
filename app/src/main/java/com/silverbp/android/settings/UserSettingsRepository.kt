...
class UserSettingsRepository(private val context: Context) : EntitlementStore {
    ...
    val flow: Flow<UserSettings> = context.dataStore.data.map { prefs ->
        UserSettings(
            ...
            weightUnit = prefs[Keys.WEIGHT_UNIT] ?: "kg",
    ...
    suspend fun setWeightUnit(raw: String) {
        context.dataStore.edit { it[Keys.WEIGHT_UNIT] = raw }
    }
    ...
    suspend fun snapshotKv(): Map<String, KvValue> {
        ...
        prefs[Keys.WEIGHT_UNIT]?.let { out[Keys.WEIGHT_UNIT.name] = KvValue.T(it) }
    ...
    suspend fun applyKvSingle(keyName: String, value: KvValue) {
        context.dataStore.edit { prefs ->
            when (keyName) {
                ...
                Keys.WEIGHT_UNIT.name -> if (value is KvValue.T) prefs[Keys.WEIGHT_UNIT] = value.value
