...
/**
 * Concrete `SyncRecordMapper` for the v19 `weight_log` table. id-keyed like
 ...
 * once cross-device weight sync ships — never renumber a tag, only append. iOS
 * BPCoach mirrors this 1:1 when it adopts weight.
 *
 * Unlike glucose there is no measure-context column, so the weight layout skips
 * that tag entirely (the tags are contiguous for this fresh table).
 *
 *   1: valueKg          (double — canonical kg; 1 lb = 0.453592 kg)
 *   2: displayUnitRaw   (string — "kg" | "lb")
 *   3: timestampMs      (int — Long ms since epoch)
 *   4: sourceRaw        (string — manual | camera)
 *   5: confidence       (double)
 *   6: note             (string)
 *   7: photoFilename    (string?)
 *   8: createdAtMs      (int)
 *   9: updatedAtMs      (int)
 *  10: memberId         (string — v19 owning member)
 *
 * `hcRecordId` stays local (platform-specific Health Connect dedup key), mirroring
 * [BpReadingSyncMapper] / [GlucoseReadingSyncMapper] / [FoodLogSyncMapper].
 *
 ...
 *
 * Backward compat (tag 10): the weight table is born member-native so a
...
class WeightReadingSyncMapper(
...
) : SyncRecordMapper<WeightLogEntity> {

    private object Field {
        const val VALUE_KG = 1
        const val DISPLAY_UNIT_RAW = 2
        const val TIMESTAMP_MS = 3
        const val SOURCE_RAW = 4
        const val CONFIDENCE = 5
        const val NOTE = 6
        const val PHOTO_FILENAME = 7
        const val CREATED_AT_MS = 8
        const val UPDATED_AT_MS = 9
        const val MEMBER_ID = 10
    }

    override fun encode(entity: WeightLogEntity, hlc: Hlc): SyncRecord {
        val payload = mutableMapOf<Int, SyncValue>(
            Field.VALUE_KG to SyncValue.Double(entity.valueKg),
        ...
        return SyncRecord(
            type = SyncEntityType.WEIGHT_LOG,
            pk = entity.id,
            hlc = hlc,
            deletedAt = null,
            payload = payload,
    ...
    override suspend fun apply(record: SyncRecord) {
        require(record.type == SyncEntityType.WEIGHT_LOG) {
        ...
        if (record.isTombstone) {
            ...
            syncDao.upsertTombstone(
                TombstoneEntity(
                    entityType = SyncEntityType.WEIGHT_LOG.tableName,
                    pk = record.pk,
